...
- **智能删除**：关闭标签仅从本工具列表移除引用，**不会删除磁盘文件**
- **同名支持**：使用完整路径存储，支持同名文件共存
- **自动保存**：输入停止 0.8 秒后自动保存，防止数据丢失
- **手动保存**：支持 `Ctrl+S` / `Cmd+S` 快捷键
- **会话恢复**：自动记忆上次编辑的笔记
- **本地存储**：默认保存在 `~/.ytools/` 目录（可在设置中修改）
- **代码编辑器**：基于 CodeMirror 6，支持 Markdown 高亮、当前行高亮、`Ctrl+F` 查找
- **JSON 自动格式化**：向空白笔记粘贴 JSON 时自动美化缩进
